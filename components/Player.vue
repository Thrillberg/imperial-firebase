  ...
  <v-col>
    ...
    >
      <v-card-item>
        ...
        <v-card-subtitle>
          <div>
            ${{ player.cash }}mil
            <span
              v-if="player.name === game.instance.investorCardHolder && !someoneIsPurchasingABond"
      ...
      <v-card-text class="overflow-auto">
        ...
        />
        <div
          v-if="player.name === game.instance.investorCardHolder"
          class="font-weight-black"
        >
          Investor Card
        </div>
        <div v-if="game.instance.swissBanks.includes(player.name)">
          Swiss Bank
        </div>
      </v-card-text>
    </v-card>
...
export default {
  ...
  computed: {
    someoneIsPurchasingABond() {
      const purchasingBond = this.game.instance.availableActions.size > 0
        && Array.from(this.game.instance.availableActions).every(
  ...
  methods: {
    ...
    canTradeIn(bond) {
      ...
      for (const availableBond of this.game.instance.availableActions) {
      ...
      if (
        this.purchasingBond
        && this.game.instance.players[this.currentPlayer].bonds.has(bond)
    ...
    controlledNations(playerName) {
      let out = [];
      for (const [nation] of this.game.instance.nations) {
        if (this.game.instance.nations.get(nation).controller === playerName) {

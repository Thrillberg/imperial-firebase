    ...
    <v-row>
      ...
      <v-col
        v-if="!paused && game.importing
          && !chooseImportType
          && (user.displayName === controllingPlayerName || (game.soloMode && hostingThisGame))"
      >
        <v-btn
          color="error"
  ...
  computed: {
    purchasingBond() {
      ...
      return (
        purchasingBond
        && (
          this.user.displayName === this.controllingPlayerName
    ...
    canForceInvestor() {
      if (this.game.availableActions.size > 0
        ...
        if (
          this.game.swissBanks.includes(this.user.displayName)
          || (this.game.soloMode && this.hostingThisGame)
        ) {
          return true;
        }
    ...
    canEndManeuver() {
      ...
      && (
        this.user.displayName === this.game.currentPlayerName
          || (this.game.soloMode && this.hostingThisGame)
      );
    },
  },
  methods: {
    ...
    canUndo() {
      ...
      for (const action of this.game.availableActions) {
        if (
          action.type === 'undo' && (
            action.payload.player === this.user.displayName
            || (this.game.soloMode && this.hostingThisGame)
          )
        ) {
          canUndo = true;

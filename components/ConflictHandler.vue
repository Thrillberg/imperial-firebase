<template>
  <v-col
    v-if="game.handlingConflict && (user.displayName === controllingPlayerName || (game.soloMode && hostingThisGame))"
  >
    <v-row
      v-if="fighting()"
  ...
  <v-col
    v-else-if="fighting() && (user.displayName === controllingPlayerName || (game.soloMode && hostingThisGame))"
    class="text-center"
  >
    <v-btn
      v-for="fightAction in fightActions()"

...
export default {
  ...
  computed: {
    purchasingBond() {
      ...
      return purchasingBond
        && (this.user.displayName === this.controllingPlayerName || (this.game.instance.soloMode && this.hostingThisGame));
    },
    sortedPlayers() {
      if (this.game.winner === '') {
        return this.players();
  ...
  methods: {
    players() {
      const players = {};
      for (const name in this.game.instance.players) {
        players[name] = this.game.instance.players[name];
      }
